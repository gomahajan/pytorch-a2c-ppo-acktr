...
if __name__ == "__main__":
    ...
    for game in games:
        ...
        files = ["3-20180801-170454-gaurav-msi-64-2-"]
        X, y, shape, N = generateData(filename=files[0], data_dir=data_dir, loading=False, N=10000,
                                      save_rate=1000000000, fileSave=file, env_name=game)
        X = X.view(N, shape).numpy()
        y = y.view(N).numpy()
        m,l = lfcn(game, file)
        print("LFCN: {} and mean {}".format(m,l))